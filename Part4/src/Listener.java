...
public class Listener extends LittleBaseListener {
    ...
    public void exitProgram(LittleParser.ProgramContext ctx) {
        ...
        try {
            ...
            if (!pgm_bdy.callBackName.equals("pgm_bdy") || !id.callBackName.equals("id")) {
                ...
                System.out.println(";Actual pattern: " + id.callBackName + " with data: " + id.data + " \n;and " + pgm_bdy.callBackName + " with data: " + pgm_bdy.data);
    ...
    public void exitFunc_decl(LittleParser.Func_declContext ctx) {
        ...
        try { // func_name func_body
            ...
            if (func_body.callBackName.equals("func_body") && func_name.callBackName.equals("func_name")) {
                ...
                System.out.println(
                        "Error: callBackName for node in exit func_decl expr was not 'func_name' or 'func_body'. Actual callbacks: " + func_body.callBackName + " and " + func_name.callBackName);
                System.out.println("expr is " + func_name.data);
            }
        } catch (EmptyStackException e) {
    ...
    public void exitStmt(LittleParser.StmtContext ctx) {
        ...
        if (this.nodeStack.peek().callBackName.equals("base_stmt")) {
            ...
            System.out.println(
                    "Error: in exit stmt the nodeStack.pop.callBackName wasn't base_stmt or if_stmt or while_stmt. Actual callback: " + this.nodeStack.peek().callBackName);
        }
    }
    ...
    public void enterIf_stmt(LittleParser.If_stmtContext ctx) {
        ...
        Blockdepth++;
        ASTNode<String> ifNode = new ASTNode<String>("if_stmt", "IF");
        //ASTNode
    ...
    public void exitIf_stmt(LittleParser.If_stmtContext ctx) {
        this.scopeStack.pop();
        ASTNode possIf = this.nodeStack.pop();
        ArrayList<ASTNode> ifChildren = new ArrayList<>();
        while(!possIf.callBackName.contentEquals("if_stmt")){
            ifChildren.add(0, possIf);
            possIf = this.nodeStack.pop();
        }

        //Add all of the if statement's children
        for (ASTNode child : ifChildren){
            possIf.childrenList.add(child);
        }
        this.nodeStack.push(possIf);
    ...
    public void enterElse_part(LittleParser.Else_partContext ctx) {
        ...
        ASTNode<String> elsePart = new ASTNode<String>("else", "ELSE");
        this.nodeStack.push(elsePart);
    }
    ...
    public void exitElse_part(LittleParser.Else_partContext ctx) {
        ...
        ASTNode possElse = this.nodeStack.pop();
        ArrayList<ASTNode> elseChildren = new ArrayList<>();
        while(!possElse.callBackName.contentEquals("else")){
            elseChildren.add(0, possElse);
            possElse = this.nodeStack.pop();
        }

        //Add all of the if statement's children
        for (ASTNode child : elseChildren){
            if(child.callBackName.contentEquals("stmt_list")){
                ArrayList<ASTNode> subChildren = child.childrenList;
                for(ASTNode singleStatement : subChildren){
                    possElse.childrenList.add(singleStatement);
                }
            } else {
                possElse.childrenList.add(0, child);
            }
        }
        if(!(possElse.childrenList.size()==0)){
            this.nodeStack.push(possElse);
        }
    }
    ...
    public void exitCond(LittleParser.CondContext ctx) {
        ASTNode expr2 = this.nodeStack.pop();
        ASTNode expr1 = this.nodeStack.pop();
        String compSymbol = ctx.getChild(1).getText();
        ASTNode compNode = new ASTNode("comp", compSymbol);
        compNode.childrenList.add(expr1);
        compNode.childrenList.add(expr2);
        ASTNode controlNode = this.nodeStack.pop();
        if (!(controlNode.callBackName.contentEquals("if_stmt") || controlNode.callBackName.contentEquals("while_stmt"))){
            System.out.println(";Error building if statement. Expected if node was actually " + controlNode.callBackName);
        } else {
            controlNode.childrenList.add(compNode);
            this.nodeStack.push(controlNode);
        }
    }
    ...
    public void enterWhile_stmt(LittleParser.While_stmtContext ctx) {
        ...
        this.scopeStack.push(block);
        Blockdepth++;
        ASTNode<String> whileNode = new ASTNode<>("while_stmt", "WHILE");
    ...
    public void exitWhile_stmt(LittleParser.While_stmtContext ctx) {
        this.scopeStack.pop();
        ASTNode<String> possWhile = this.nodeStack.pop();
        ArrayList<ASTNode> children = new ArrayList<ASTNode>();
        while(!possWhile.callBackName.contentEquals("while_stmt")){
            if (possWhile.callBackName.contentEquals("stmt_list")){
                ArrayList<ASTNode> subChildren = possWhile.childrenList;
                for(ASTNode child : subChildren){
                    children.add(child);
                }
            } else {
                children.add(0, possWhile);
            }
            possWhile = this.nodeStack.pop();
        }

        //Add all of the if statement's children
        for (ASTNode child : children){
            possWhile.childrenList.add(child);
        }
        this.nodeStack.push(possWhile);
    }
